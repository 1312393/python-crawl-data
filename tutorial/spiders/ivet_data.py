...
class IvetDataSpider(scrapy.Spider):
    ...
    def parse_page(self, response):
        ...
        for data in url_selector:
            if data.css('a::text').extract() in [['Regulatory Decision Information'], ['Delivery'], ['Scope']]:
                ...
                yield SplashRequest("https://training.gov.au" + data.css('a::attr(href)').extract()[0], callback = self.parse_item,
                                    endpoint='execute',
                                    args={'lua_source': script3})
    ...
    def parse_item(self, response):
        ...
        for quote in response.css('div.display-row'):
            data = {
                w3lib.html.strip_html5_whitespace(quote.css('div.display-label::text').extract_first() or ''):
        ...
        #     yield {
        #         w3lib.html.strip_html5_whitespace(quote.css('div.display-label::text').extract_first() or ''):
        #             w3lib.html.strip_html5_whitespace(quote.css('div.display-field-no-width::text').extract_first() or quote.css('div.display-field-unblocked-narrowest::text' or '').extract_first())
        #     }
        pass